...
<link rel="stylesheet" href="./styles/legacy-core.css">
<link rel="stylesheet" href="./styles/premium-refinement-v58.css">
<link rel="stylesheet" href="./styles/global-design-system-v59.css">
<link rel="stylesheet" href="./styles/audit-fixes-v57.css">
<link rel="stylesheet" href="./styles/product-system-v60.css">
<link rel="stylesheet" href="./styles/qa-final-v61.css">
...
,
  ...
  function login(){mount(`<main class="login-screen"><div class="login-shell"><section class="login-card"><div class="login-brand"><div><div class="access-indicator" aria-label="Centro de performance privado">
  <span class="access-icon" aria-hidden="true">⌁</span>
  <span class="access-main">Centro de performance</span>
  <span class="access-divider"></span>
  <span class="access-owner">Treinador Esteves</span>
  ...
  async function logout(){await supabaseLogout(); sessionStorage.removeItem('hero_session'); login();}
  ...
  function floatingAIButton(){
    const s=sessionStorage.getItem('hero_session');
    if(!s)return '';
    try{
      const u=JSON.parse(s);
      if(u.role!=='admin')return '';
      const ctx=currentTrainerContext();
      return `<button class="floating-ai-btn no-print" onclick="HERO.abrirChatIA('${ctx.alunoId||''}','${ctx.treinoId||''}')"><span>IA</span> Chat HERO</button>`;
    }catch(_e){return '';}
  }
...
async function student(alunoId,preview=false){let d=data(); const a=d.alunos.find(x=>x.id===alunoId); if(!a)return login(); let ts=d.treinos.filter(t=>t.alunoId===alunoId&&(preview||t.liberado)); if(supabaseAvailable()){for(const t of ts){await syncExerciciosPrescritosSupabase(t.id,alunoId);} d=data(); ts=d.treinos.filter(t=>t.alunoId===alunoId&&(preview||t.liberado));} mount(`${top(preview?'Visualização do aluno':'Área individual do aluno',preview?`<button class="btn secondary small" id="backPrev">Voltar ao centro de controle</button>`:`<button class="btn small" onclick="window.print()">Imprimir minha missão</button><button class="btn secondary small" onclick="HERO.community()">Comunidade HERO</button>`)}<main class="wrap student-view"><section class="hero print-head"><div class="student-profile"><div class="avatar-wrap">${a.foto?`<img src="${a.foto}" alt="Foto de ${esc(a.nome)}">`:`<div class="avatar-placeholder">H</div>`}</div><div><p class="eyebrow">Protocolo liberado</p><h1>${esc(a.nome)}</h1><p>Seu treino foi criado para estética, força, resistência e evolução real.</p><div class="mission-ribbon no-print">Área exclusiva HERO liberada</div><div class="toolbar no-print">${preview?'':`<button class="btn small" id="photoBtn">${a.foto?'Alterar foto':'Adicionar foto'}</button>`}<button class="btn" onclick="window.print()">Imprimir minha missão</button></div></div></div></section>${heroControlMetrics(a.id)}<section class="print-area">${ts.length?studentToday(alunoId,ts,preview)+ts.map(t=>studentWorkout(t,alunoId,preview)).join(''):'<div class="empty">Sua missão ainda não foi liberada.</div>'}</section></main>`); const b=$('backPrev'); if(b)b.onclick=()=>admin(alunoId); const pb=$('photoBtn'); if(pb)pb.onclick=()=>photoEditor(alunoId); if(!preview) initTimer(alunoId);}
  ...
  async function community(){
    ...
    if(backLegacy)backLegacy.onclick=function(ev){ev.preventDefault();ev.stopPropagation();location.hash='';if(isAdmin)return admin();if(user.id&&user.id!=='local-user')return student(user.id,false);return login();};
    const protocolLegacy=$('communityProtocolDirectLegacy');
    if(protocolLegacy)protocolLegacy.onclick=function(ev){ev.preventDefault();ev.stopPropagation();location.hash='';if(user.id&&user.id!=='local-user')return student(user.id,false);return login();};
...
(function(){
  ...
  function currentUser(){try{var raw=sessionStorage.getItem('hero_session');var u=raw?JSON.parse(raw):{};var st=typeof data==='function'?data():{};var aluno=(st.alunos||[]).find(function(a){return String(a.id)===String(u.alunoId)||String(a.email||'').toLowerCase()===String(u.email||'').toLowerCase()});return {id:u.alunoId||u.id||u.email||(aluno&&aluno.id)||'local-user',name:u.nome||u.name||(aluno&&aluno.nome)||u.email||'Usuário HERO',role:u.role||(aluno&&aluno.role)||'aluna',email:u.email||(aluno&&aluno.email)||''};}catch(e){return {id:'local-user',name:'Usuário HERO',role:'aluna',email:''};}}
  ...
  async function uploadImg(file){if(!file)return '';if(typeof supabaseClient!=='function'||typeof supabaseAvailable!=='function'||!supabaseAvailable())return '';var client=supabaseClient();if(!client)return '';var ext=(file.name.split('.').pop()||'jpg').toLowerCase();var path='posts/'+Date.now()+'-'+(typeof uid==='function'?uid():Math.random().toString(36).slice(2))+'.'+ext;var up=await client.storage.from('community-media').upload(path,file,{cacheControl:'3600',upsert:false});if(up.error)throw new Error('Erro ao enviar imagem: '+up.error.message);var pub=client.storage.from('community-media').getPublicUrl(path);return (pub.data&&pub.data.publicUrl)||'';}
...
if(backBtn)backBtn.onclick=function(ev){ev.preventDefault();ev.stopPropagation();location.hash='';if(isAdmin&&window.HERO&&typeof window.HERO.admin==='function')return window.HERO.admin();if(user&&user.id&&user.id!=='local-user'&&window.HERO&&typeof window.HERO.student==='function')return window.HERO.student(user.id,false);if(window.HERO&&typeof window.HERO.login==='function')return window.HERO.login();};
var protocolBtn=document.getElementById('communityProtocolDirect');
if(protocolBtn)protocolBtn.onclick=function(ev){ev.preventDefault();ev.stopPropagation();location.hash='';if(user&&user.id&&user.id!=='local-user'&&window.HERO&&typeof window.HERO.student==='function')return window.HERO.student(user.id,false);if(window.HERO&&typeof window.HERO.login==='function')return window.HERO.login();};
...
(function(){
  ...
  function currentStudentId(){
    ...
    return s.alunoId||s.id||(aluno&&aluno.id)||s.email||'';
  }
  ...
  function isOldCommunityButton(el){
    ...
    if(el.closest&&el.closest('.community-context-bar'))return false;
  ...
  function ensureStudentAI(){
    if(isTrainer())return;
    const studentArea=!!document.querySelector('.hero-load-box,.student-ex,.workout-card,.protocol-exercise,.today-panel');
  ...
  function bindCommunityNav(){
    ['communityBackDirect','communityBackDirectLegacy','communityProtocolDirect','communityProtocolDirectLegacy'].forEach(id=>{
      ...
      if(el){
        el.onclick=function(ev){ev.preventDefault();ev.stopPropagation();if(ev.stopImmediatePropagation)ev.stopImmediatePropagation();return goCommunityBack();};
  ...
  function ensureCheckin(){
    if(document.getElementById('heroCheckinStableButton'))return;
    const card=findByTitle(['desafio','semana'])||findByTitle(['check-in'])||findByTitle(['checkin'])||findByTitle(['desafio']);
    if(!card&&!isCommunityScreen())return;
    const btn=document.createElement('button');
    btn.type='button'; btn.id='heroCheckinStableButton'; btn.className='hero-checkin-action-clean';
    btn.innerHTML='<span>Check-in HERO<small>Toque aqui para registrar seu check-in no desafio da semana.</small></span><b>→</b>';
    btn.onclick=function(ev){ev.preventDefault();ev.stopPropagation();return triggerCheckin();};
    if(card)card.appendChild(btn); else {const sec=document.createElement('section');sec.className='card hero-community-clean-section';sec.innerHTML='<h3>Desafio da semana</h3>';sec.appendChild(btn);root().appendChild(sec);}
  }
  ...
  document.addEventListener('click',function(ev){
    ...
    if(id==='communityBackDirect'||id==='communityBackDirectLegacy'||id==='communityProtocolDirect'||id==='communityProtocolDirectLegacy'){
      ev.preventDefault();ev.stopPropagation();if(ev.stopImmediatePropagation)ev.stopImmediatePropagation();return goCommunityBack();
    }
    scheduleStable();
  },true);
...
(function(){
  ...
  function bindCommunityNavigation(){
    ['communityBackDirect','communityBackDirectLegacy','communityProtocolDirect','communityProtocolDirectLegacy'].forEach(function(id){
      ...
      el.addEventListener('click',function(ev){ev.preventDefault();ev.stopPropagation();if(ev.stopImmediatePropagation)ev.stopImmediatePropagation();var s=safeSession();try{location.hash='';}catch(e){} if(isTrainer()&&window.HERO&&typeof window.HERO.admin==='function')return window.HERO.admin();var sid=s.alunoId||s.id||s.email||'';if(window.HERO&&typeof window.HERO.student==='function')return window.HERO.student(sid,false);if(window.HERO&&typeof window.HERO.login==='function')return window.HERO.login();},true);
    });
